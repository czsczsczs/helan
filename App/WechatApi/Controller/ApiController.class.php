...
class ApiController extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach ($activity as $key1 => $value1) {
            $activity[$key1]['cover'] = 'https://www.hollandshow.cn'.$value1['cover_width_300'];
        ...
        foreach ($shop as $key2 => $value2) {
            $shop[$key2]['cover'] = 'https://www.hollandshow.cn'.$value2['cover_width_300'];
        ...
        foreach ($post as $key3 => $value3) {
            $post[$key3]['cover'] = 'https://www.hollandshow.cn'.$value3['cover_width_300'];
    ...
    public function lists($type)
    {
        ...
        if($type == 'activity'){
            ...
            $media = $this->mediaModel->where($where)
                        ->order('created desc')
                        ->select();
                        foreach ($media as $nkey => $nvalue) {
                            $media[$nkey]['type'] = 'media';

                        }
                        $media2 = $this->media2Model->where($where)
                                                ->order('created desc')
                                                ->select();
                                                foreach ($media2 as $nkey => $nvalue) {
                                                    $media2[$nkey]['type'] = 'media2';
        ...
        foreach ($lists as $key => $value) {
            $lists[$key]['cover'] = 'https://www.hollandshow.cn'.$value['cover_width_300'];
...
//     <img src="https://www.hollandshow.cn/Public/upload/2018-10-27/5bd42a2c0c6fb.JPG" width="600" height="335" style="width: 600px; height: 335px;" border="0" vspace="0" title="" alt=""/>
// </p>
            // $data['cover'] = 'https://www.hollandshow.cn'.$data['cover_width_300'];
            /*$data['cover'] = '<p style="text-align: center;"><img src="https://www.hollandshow.cn'.$data['cover'].'"  style="100%" border="0" vspace="0" title="" alt=""></p>';
        ...
        // $data['content'] = str_replace('src="/','src="https://www.hollandshow.cn/',htmlspecialchars_decode($data['content']));
        $data['content'] = str_replace('src="/','src="https://www.hollandshow.cn/',htmlspecialchars_decode($data['content']));
        ...
        if($data['images']){
            ...
            foreach ($imagesId as $key => $value) {
                ...
                $data['image'][$key]['savepath'] = 'https://www.hollandshow.cn/'.$file['save_path'];
            }
        }
    ...
    public function search($keyword){
        ...
        $media2 = $this->media2Model->where($where)
                ...
                foreach ($media2 as $nkey => $nvalue) {
                    $media2[$nkey]['type'] = 'media2';
                    $media2[$nkey]['type_'] = '視頻集';
                }
        $lists = array_merge($news,$shop,$active,$post,$street,$media,$media2);
        foreach ($lists as $key => $value) {
            $lists[$key]['cover'] = 'https://www.hollandshow.cn'.$value['cover_width_300'];
            $lists[$key]['content'] = htmlspecialchars_decode($value['content']);
            $lists[$key]['created'] = date('Y-m-d',$value['created']);
    ...
    public function banner($id){
        ...
        $data = $this->slideModel->where($where)
            ->find();
        $data['image'] = 'https://www.hollandshow.cn'.$data['image'];
        return $this->ajaxReturn(success($data));
    }
